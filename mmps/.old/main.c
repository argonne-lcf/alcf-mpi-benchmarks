...
/* Returns MMPS */
inline double measureMMPS( void )
{
  ...
  if ( layer_rank == 0 ) 
  {
      ...
      for ( iter = 0; iter < MAXN; iter ++ )
      {
          ...
          MPI_Waitall( 2 * num_targets, req, MPI_STATUSES_IGNORE );
      }
      t = MPI_Wtime() - t;

      mmps =  (double)( 2 * num_targets * MAXN  ) / t / 1e6;
  }
  else
  {
      /* receiver part */
...
int main( int argc, char *argv[] )
{
 ...
 // int ppn, num_targets, taskid, ntasks, source, i, j, window, receiver, best_win, provided;
  int taskid, ntasks, provided, source, rc, color;
