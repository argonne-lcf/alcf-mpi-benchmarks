...
extern MPI_Comm chasiss_comm, cabinet_comm, intragroup_comm, intergroup_comm;
extern int chasiss_size, chasiss_rank, cabinet_size, cabinet_rank, intragroup_rank, intragroup_size, intergroup_size, intergroup_rank;
...
int init()
{
  ...
  */
  ...
  /* node_comm include all processes on the same node [0-hop] */
  MPI_Comm_split( MPI_COMM_WORLD, node_enc, rank, &node_comm );
  MPI_Comm_size( node_comm, &node_size );
  MPI_Comm_rank( node_comm, &node_rank );

  /* aries_com includes all processes connected to the same aries chip [0-hop] */
  ...
  /* chassis_comm includes all processes within a same chassis (connected with green links) 
     [1-hop green link Rank-1] */
  MPI_Comm_split( MPI_COMM_WORLD, node_enc & 0x1FC3, rank, &chasiss_comm );
  MPI_Comm_size(chasiss_comm, &chasiss_size );
  MPI_Comm_rank(chasiss_comm, &chasiss_rank );
  
   
  /* black_comm includes all processes within a group (vertically connected with black lines 
    refer fig.8 http://www.cray.com/sites/default/files/resources/CrayXCNetwork.pdf 
   [1-hop black link Rank-2] */
  /*MPI_Comm_split( MPI_COMM_WORLD, node_enc & 0x1E3F, rank, &_comm );
  MPI_Comm_size(cabinet_comm, &cabinet_size );
  MPI_Comm_rank(cabinet_comm, &cabinet_rank );
 */
  /* intragroup_comm includes all processes within a same group [not connected over 1 all-to-all] (within 6 chassis) */
  MPI_Comm_split( MPI_COMM_WORLD, node_enc & 0x1E00, rank, &cabinet_comm );
  MPI_Comm_size(cabinet_comm, &cabinet_size );
  MPI_Comm_rank(cabinet_comm, &cabinet_rank );
 
 /* intergroup_comm includes all processes across different groups (3-5 hops)*/
  MPI_Comm_split( MPI_COMM_WORLD, node_enc & 0x1E00, rank, &cabinet_comm );
  MPI_Comm_size(cabinet_comm, &cabinet_size );
  MPI_Comm_rank(cabinet_comm, &cabinet_rank );
