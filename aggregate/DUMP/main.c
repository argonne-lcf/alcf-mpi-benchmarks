...
MPI_Status stat[2 * XNBORS * MAXTASKS];
     ...
     for ( k = 0; k < MAXN+MAXSKIP; k++ )
      {
        ...
        for ( i = 0; i < N; i++ )
        {
          MPI_Isend( sb, LENGTH, MPI_BYTE, i+1, tag, layer_comm, &req[total++] );
          MPI_Irecv( rb, LENGTH, MPI_BYTE, i+1, tag,    layer_comm, &req[total++] );
        }
...
*/
  ...
  if ( report_rank == 0 && color < 5 )
     printf( "ppn = %d: Aries %d: nodes %d: min/max mmps  %lf / %lf GB/s\n", ppn, layer_size, report_size, mind, maxd );
